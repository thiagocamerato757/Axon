...
void test_tensor_fill_operations() {
    ...
    int all_neg = 1;
    for (u32 i = 0; i < t->size; i++)
        if (!float_eq(t->data[i], -7.5f, 1e-5f)) all_neg = 0;
    ...
    int all_zero2 = 1;
    for (u32 i = 0; i < t->size; i++)
        if (!float_eq(t->data[i], 0.0f, 1e-9f)) all_zero2 = 0;
    ...
    check("Ones after zeros — last elem 1",   float_eq(t->data[t->size-1], 1.0f, 1e-9f));
...
void test_tensor_random_generation() {
    ...
    for (u32 i = 1; i < t->size; i++)
        if (!float_eq(t->data[i], t->data[0], 1e-5f)) { has_variation = 1; break; }
    check("Uniform — values are not constant", has_variation);

    // uniform — mean should be close to 0.5
    f32 sum_u = 0.0f;
    for (u32 i = 0; i < t->size; i++) sum_u += t->data[i];
    ...
    f32 var = 0.0f;
    for (u32 i = 0; i < t->size; i++) var += (t->data[i] - mean) * (t->data[i] - mean);
    ...
    int within_4std = 1;
    for (u32 i = 0; i < t->size; i++)
        if (t->data[i] < -4.0f || t->data[i] > 4.0f) within_4std = 0;
    check("Normal(0,1) — values within ±4σ",   within_4std);

    // normal — must not produce NaN values
    int no_nan = 1;
    for (u32 i = 0; i < t->size; i++)
        if (t->data[i] != t->data[i]) no_nan = 0;  // NaN != NaN
...
void test_tensor_get() {
    ...
    u32 idx1[] = {1};
    check("tensor_get 1D [1] == 7.0", float_eq(tensor_get(t1, idx1), 7.0f, 1e-5f));
    ...
    u32 idx2[] = {1, 2};
    check("tensor_get 2D [1][2] == 6", float_eq(tensor_get(t2, idx2), 6.0f, 1e-5f));
...
void test_tensor_matmul() {
    ...
    tensor_matmul(c, a, b);
    check("2x2 matmul — C[0][0] == 19", float_eq(tensor_get2d(c,0,0), 19.0f, 1e-4f));
    check("2x2 matmul — C[0][1] == 22", float_eq(tensor_get2d(c,0,1), 22.0f, 1e-4f));
    check("2x2 matmul — C[1][0] == 43", float_eq(tensor_get2d(c,1,0), 43.0f, 1e-4f));
    check("2x2 matmul — C[1][1] == 50", float_eq(tensor_get2d(c,1,1), 50.0f, 1e-4f));
    ...
    check("[2,3]@[3,4] — C[0][0]==1", float_eq(tensor_get2d(c2,0,0), 1.0f, 1e-4f));
    check("[2,3]@[3,4] — C[0][2]==5", float_eq(tensor_get2d(c2,0,2), 5.0f, 1e-4f));
    check("[2,3]@[3,4] — C[1][0]==6", float_eq(tensor_get2d(c2,1,0), 6.0f, 1e-4f));
    check("[2,3]@[3,4] — C[1][3]==4", float_eq(tensor_get2d(c2,1,3), 4.0f, 1e-4f));
    tensor_free(&a2); tensor_free(&b2); tensor_free(&c2);
    ...
    for (u32 i = 0; i < mat->size; i++)
        if (!float_eq(res->data[i], mat->data[i], 1e-4f)) id_ok = 0;
    check("A @ I == A (identity)",              id_ok);
    tensor_free(&mat); tensor_free(&eye); tensor_free(&res);
    ...
    for (u32 i = 0; i < r->size; i++)
        if (!float_eq(r->data[i], 0.0f, 1e-5f)) zero_ok = 0;
    check("A @ 0 == 0 (zero matrix)",           zero_ok);
    tensor_free(&m); tensor_free(&z); tensor_free(&r);
    ...
    for (u32 i = 0; i < zz->size; i++)
        if (!float_eq(zz->data[i], 0.0f, 1e-9f)) still_zero = 0;
    check("Incompatible shapes — out unchanged", still_zero);
    tensor_free(&x); tensor_free(&y); tensor_free(&zz);
}
