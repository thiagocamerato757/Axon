...
Tensor *tensor_create(u32 *shape, u8 ndim) {
  ...
  for (u8 i = 0; i < MAX_TENSOR_DIMS; i++)
    tensor->shape[i] = 0;
 ...
 * @param tensor: A pointer to the tensor to be initialized
 */
void tensor_ones(Tensor *tensor) { tensor_fill(tensor, 1.0f); }
...
void tensor_random(Tensor *tensor) {
  ...
  for (u32 i = 0; i < tensor->size; i++) {
    tensor->data[i] =
        (f32)rand() / (f32)RAND_MAX; // Random float between 0 and 1
  }
}
...
/**
 * This function retrieves an element from a tensor given its multi-dimensional
 * indices
 * @param tensor: A pointer to the tensor from which to retrieve the element
 * @param indices: An array of indices corresponding to each dimension of the
 * tensor
 * @return The value of the element at the specified indices, or 0.0f if the
 * tensor is invalid or the indices are out of bounds
 */
 ...
 * @return The value of the element at the specified index, or 0.0f if the
 * tensor is invalid or the index is out of bounds
 */
  ...
  for (u32 i = 0; i < rows; i++) {
    for (u32 k = 0; k < inner; k++) {
      ...
      for (u32 j = 0; j < cols; j++) {
        // acumulates the product of a_ik and the corresponding element in b
